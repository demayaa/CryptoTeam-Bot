...
async function getAudioDuration(buffer) {
    let duration;
    try {
        const mp3Duration = await Promise.resolve().then(() => __importStar(require('mp3-duration')));
        duration = await mp3Duration(buffer, true);
    }
    catch (_a) {
        const readable = new stream_1.Readable({ read() { this.push(buffer); this.push(null); } });
        const { getAudioDurationInSeconds } = await Promise.resolve().then(() => __importStar(require('get-audio-duration')));
